...
- Phase 2 known issues (2026-04-29, non-blocking): (1) Font FOUT on first load — `@fontsource-variable` packages load via CSS without preload hints; pre-existing in Phase 1, not introduced by Phase 2; defer to Phase 6 audit. (2) `vite preview`'s built-in server does not honor `Range:` headers, so `<video preload="auto">` must download the full 3.3MB MP4 before metadata is available — manifests as "video doesn't scrub for several seconds after page load" in local preview only. Vercel's production edge serves range requests; the live deploy will not exhibit this. Document in Phase 6 audit playbook so future preview-build smokes account for it.
